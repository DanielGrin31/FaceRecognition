...
function browseFiles(inputNum) {
  const input = $(`#dragarea${inputNum + 1}`).find("input");
  input.click();
}
function setCanvasSourceFromInput(inputNum) 
{
  let dragArea=$(`#dragarea${inputNum + 1}`);
  const input = dragArea.find("input")[0];
  let file=input.files[0]
  showFile(dragArea,file);
}
